...
# 更新状态栏
refresh() {
    _icons='';_netspeed='';_music='';_wifi='';_cpu='';_mem='';_date='';_vol=''# 重置所有模块的状态为空
    source $tempfile                                                     # 从 temp 文件中读取模块的状态
    xsetroot -name "$_icons$_netspeed$_music$_wifi$_cpu$_mem$_date$_vol"      # 更新状态栏
}
...
cron() {
    ...
    while true; do
        to=()                                                            # 存放本次需要更新的模块
        [ $((i % 1)) -eq 0 ]  && to=(${to[@]} netspeed)                     # 每 1秒  更新 netspeed
...
case $1 in
    ...
    updateall|check) update icons netspeed music wifi cpu mem date vol ;;
