...
execute_menu() {
    case $1 in
        ' set wallpaper')
            ...
            ;;
        '艹 update statusbar')
            coproc (~/workspace/dwm/statusbar/statusbar.sh updateall > /dev/null 2>&1)
            ;;
        ' open v2raya')
            coproc (sudo docker restart v2raya > /dev/null && ~/workspace/dwm/statusbar/statusbar.sh updateall > /dev/null)
            ;;
        ' close v2raya')
            coproc (sudo docker stop v2raya > /dev/null && ~/workspace/dwm/statusbar/statusbar.sh updateall > /dev/null)
            ;;
        ' open picom')
